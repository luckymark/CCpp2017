...
void Init()
{
    ...
    srand(time(NULL));
    for(int i=0;i<=M;i++)fa[i]=i,rk[i]=1;
...
void PreMaze()
{
    for(int i=0;i<N;i++)maze[0][i]=1;
    for(int i=0;i<N;i++)maze[N-1][i]=1;
    for(int i=0;i<N;i++)maze[i][0]=1;
    for(int i=0;i<N;i++)maze[i][N-1]=1;
    cnt=0;cnt2=1;
    for(int i=1;i<N-1;i++)
    {
        for(int j=1;j<N-1;j++)
        {
            if((i%2==0&&j%2)||(j%2==0))maze[i][j]=1;
            else
            {
                node[cnt2].x=i;node[cnt2++].y=j;
            }
            if((j%2&&i%2==0)||(j%2==0&&i%2))edge[cnt].x=i,edge[cnt++].y=j;
        }
    }
}
int GetRandArc()
...
int FindPos(Pos cur)
{
    ...
    {
        if(node[i].x==cur.x&&node[i].y==cur.y)return i;
    }
}
    ...
    for(int i=0;i<N;i++)
    {
        for(int j=0;j<N;j++)
        {
            if(i==x&&j==y)
            {
                ...
                if(maze[i][j])printf("%c%c",0xa8,0x80);
                else printf("  ");
            }
...
void GenerateRandomMaze()
{
    ...
    while(Find(1)!=Find(cnt2-1))
    {
        ...
        if(edge[temp].x%2==0)
        {
            Pos up;up.x=edge[temp].x+dirX[3];up.y=edge[temp].y+dirY[3];
            Pos down;down.x=edge[temp].x+dirX[1];down.y=edge[temp].y+dirY[1];
        ...
        {
            Pos left;left.x=edge[temp].x+dirX[2];left.y=edge[temp].y+dirY[2];
            Pos right;right.x=edge[temp].x+dirX[0];right.y=edge[temp].y+dirY[0];
