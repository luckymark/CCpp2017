...
void Pre()
{
    ...
    {
        struct Item item;item.num=num;item.sum=sum;
...
{
    struct Item item;item.num=a;item.sum=b;
    AddToLinkedList(item);
}
void Remove(int x)
{
    ...
    int flag=0;
    for(pos=head;pos!=NULL;pos=pos->next) if(pos->item.num==x)flag=1;
    if(flag==0)
    {
        printf("not founded\n");
        return;
